...
def remove_markup(str)
  ...
  str.tr("\t","").
      ...
      gsub(/\{\{extinct\}\}/,''). # STOL does not use extinction data
      gsub(/<br ?\/?>/i," / "). # line break == two alternate values for a metadata key
      gsub(/''+/,"")  # remove italics/bolding from text
end

STDIN.each {|line|
  title, taxobox = Oj.load(line)
  taxoboxKVs = remove_markup(taxobox).scan(TaxoboxKVFormat).map {|meta, meta_value| [meta, meta_value.sub(/\{\{.+/,'')] }
